...
//Recherche du droit d'accès correspondant
    ...
    public static function BuildCorrectionChapitre($paramIdFta, $paramIdChapitre, $option) {
        ...
        $return = Chapitre::BuildDevalidationChapitre($paramIdFta, $idFtaProcessus, $HtmlResult);
        ...
        if ($return) {
            foreach ($return["processus"] as $id_Fta_Processus) {
                ...
                $arrayFtaProcessus = DatabaseOperation::convertSqlQueryWithAutomaticKeyToArray(
                                "SELECT " . FtaProcessusModel::FIELDNAME_NOM
                                . " FROM " . FtaProcessusModel::TABLENAME
                                . ", " . FtaWorkflowStructureModel::TABLENAME
                                . " WHERE " . FtaWorkflowStructureModel::TABLENAME . "." . FtaWorkflowStructureModel::FIELDNAME_ID_FTA_PROCESSUS . "=" . FtaProcessusModel::TABLENAME . "." . FtaProcessusModel::KEYNAME
                                . " AND " . FtaWorkflowStructureModel::FIELDNAME_ID_FTA_CHAPITRE . "=" . $paramIdChapitre
                );
        ...
        if ($arrayProcessusCycle != NULL) {
            foreach ($arrayProcessusCycle as $rowsProcessusCycle) {
                ...
                if (mysql_affected_rows()) { //Si le processus a été dévalidé, alors on informe
                    ...
                    $return["processus"][] = $paramIdProcessus;
                   // $htmlResult->setProcessus($paramIdProcessus);
                    ...
                    if ($arrayMail) {
                        foreach ($arrayMail as $rowsMail) {
                          ...
                          //  $htmlResult->setMail($rowsMail[UserModel::FIELDNAME_MAIL]);
                        }
                    }
                ...
                $sub_return = Chapitre::BuildDevalidationChapitre($paramIdFta, $paramIdProcessus, $htmlResult);

                //Fusion du tableaux de résultat contenant la liste des adresses email
                /* @var $subReturn type */
                if ($subReturn != NULL) {
                    $return = array_merge_recursive($return, $sub_return);
                    $return[UserModel::FIELDNAME_MAIL] = array_unique(array_merge_recursive($return[UserModel::FIELDNAME_MAIL], $sub_return[UserModel::FIELDNAME_MAIL]));
                    $return["processus"] = array_unique(array_merge_recursive($return["processus"], $sub_return["processus"]));
                    //$htmlResult->setArrayResult($return);
                }
                //print_r($return["mail"]);
            }
        }
...
//            UserModel::FIELDNAME_MAIL => $htmlResult->getMail()
//        );
