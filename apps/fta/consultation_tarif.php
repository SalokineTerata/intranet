...
//Période de recherche (Début)
          $champ="periode_debut";
          if(!$$champ) //Par défaut, début d'année en cours
          {
            $$champ=date("Y")."-01-01";
          }
          $HTML_entete.="<tr><td>Début de la période</td><td>".calendrier($champ, ${$champ})."</td></tr>";
...
//Période de recherche (Début)
          ...
          $HTML_entete.="<tr><td>Fin de la période</td><td>".calendrier($champ, ${$champ})."</td></tr>";
...
//Création du tableau de résultat
          ...
          $HTML_result .= "<tr class=titre><td>".mysql_field_desc("access_arti2", "LIBELLE", 1)
                       . "</td><td>".mysql_field_desc("access_arti2", "Poids_ELEM", 1)
                       . "</td><td>".mysql_field_desc("access_arti2", "CODE_ARTICLE", 1)
                        . "</td><td>".mysql_field_desc("access_arti2", "Site_de_production", 1)
                        . "</td><td>".mysql_field_desc("access_arti2", "EAN_UVC", 1)
                        . "</td><td>".mysql_field_desc("access_arti2", "NB_UNIT_ELEM", 1)
                        . "</td><td>".mysql_field_desc("fta_tarif", "prix_fta_tarif", 1)
                        . "</td><td>Périodes"
                        . "</td></tr>"
                        ;
          ...
          $req = "SELECT LIBELLE, Poids_ELEM, CODE_ARTICLE, Site_de_production, EAN_UVC,NB_UNIT_ELEM, prix_fta_tarif, date_debut_fta_tarif, date_fin_fta_tarif  "
               . "FROM access_arti2, fta, fta_etat, fta_tarif, geo "
               . "WHERE access_arti2.id_fta=fta.id_fta "          //Liaison
               . "AND fta.id_fta_etat=fta_etat.id_fta_etat "      //Liaison
               . "AND fta_tarif.id_fta=fta.id_fta "               //Liaison
               . "AND geo.id_site=access_arti2.Site_de_production " //Liaison
