<?php
/* $Id: display_tbl.lib.php,v 1.63.2.5 2002/06/11 15:09:55 rabus Exp $ */
...
if (!defined('PMA_DISPLAY_TBL_LIB_INCLUDED')){
    ...
    {
//Modifier par Boris Sanègre 2003.04.04
...
<tr>
        ...
        if($GLOBALS[consultation])
        {
            $is_display['text_btn']=0;
        }
        $text_url = 'sql.php'
                  . '?lang=' . $lang
...
<!-- Results table body -->
        ...
        while ($row = mysql_fetch_array($dt_result)) {
            ...
            $bgcolor = ($row_no % 2) ? $GLOBALS['cfgBgcolorOne'] : $GLOBALS['cfgBgcolorTwo'];

            if ($disp_direction == 'horizontal') {
                // loic1: pointer code part
                $on_mouse     = '';
                if ($GLOBALS['cfgBrowsePointerColor'] != '') {
                    $on_mouse = ' onmouseover="setPointer(this, ' . $row_no . ', \'over\', \'' . $bgcolor . '\', \'' . $GLOBALS['cfgBrowsePointerColor'] . '\', \'' . $GLOBALS['cfgBrowseMarkerColor'] . '\');"'
                              . ' onmouseout="setPointer(this, ' . $row_no . ', \'out\', \'' . $bgcolor . '\', \'' . $GLOBALS['cfgBrowsePointerColor'] . '\', \'' . $GLOBALS['cfgBrowseMarkerColor'] . '\');"';
                }
                if ($GLOBALS['cfgBrowseMarkerColor'] != '') {
                    $on_mouse .= ' onmousedown="setPointer(this, ' . $row_no . ', \'click\', \'' . $bgcolor . '\', \'' . $GLOBALS['cfgBrowsePointerColor'] . '\', \'' . $GLOBALS['cfgBrowseMarkerColor'] . '\');"';
                }
                ?>
...
/*
Modifié par Boris Sanègre 2003.04.07
*/
                $url_query  = 'lang=' . $lang
                            . '&amp;server=' . $server
                            . '&amp;db=' . urlencode($db)
